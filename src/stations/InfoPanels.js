...
const setup = (ctx, hall) => {
  for (let i = 0; i < INFO_DATA.length; i++) {
    ...
    const panelWidth =
      panels[i].geometry.boundingBox.max.x -
      panels[i].geometry.boundingBox.min.x
    const panelHeight =
      panels[i].geometry.boundingBox.max.y -
      panels[i].geometry.boundingBox.min.y
    const offsety = INFO_DATA[i].offsety || 0
    panels[i].material = new THREE.MeshBasicMaterial({
      color: 0x040404,
      transparent: true
    })
